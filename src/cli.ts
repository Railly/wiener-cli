...
program
  .name("wiener")
  .description("Agent-first CLI for Universidad Norbert Wiener (Canvas LMS)")
  .version("0.1.0");

// Global flags applied to all subcommands by adding them to each subcommand.
// Commander v14 doesn't propagate parent options to subcommand actions cleanly,
// so each subcommand declares the flags it needs.
...
addGlobalFlags(
  tareas
    .command("hoy")
    .description("Tareas que vencen hoy o están atrasadas")
...
addGlobalFlags(
  archivos
    .command("arbol <ref>")
    .description("Árbol de carpetas y archivos")
...
addGlobalFlags(
  program
    .command("modulos <ref>")
    .description("Módulos del curso con items")
...
addGlobalFlags(
  program
    .command("syllabus <ref>")
    .description("Silabo del curso")
).action(async (ref, opts) => {
  const g = parseGlobalFlags(opts);
...
addGlobalFlags(
  program
    .command("conferencias <ref>")
    .description("Conferencias y grabaciones del curso")
).action(async (ref, opts) => {
  const g = parseGlobalFlags(opts);
...
addGlobalFlags(
  ...
  const g = parseGlobalFlags(opts);
  if (opts["ics"]) {
    await runCalendarioIcs({ ...g, out: opts["out"] as string | undefined, curso: opts["curso"] as string | undefined });
  } else {
...
// ─── wiener canvas <name> (mirror namespace) ────────────────────────────────

const canvas = new Command("canvas")
  .description("Canvas LMS commands (espejo de top-level, misma funcionalidad)");
...
addGlobalFlags(
  ...
  const g = parseGlobalFlags(opts);
  if (ref) { await runTareasByCourse(String(ref), g); } else { await runTareasList(g); }
});
...
addGlobalFlags(
  ...
  const g = parseGlobalFlags(opts);
  if (ref) { await runCalificacionesDetail(String(ref), g); } else { await runCalificaciones(g); }
});
...
addGlobalFlags(
  ...
  const g = parseGlobalFlags(opts);
  const n = parseInt(String(opts["ultimos"] ?? "5"), 10);
  if (ref) { await runAnunciosByCourse(String(ref), { ...g, ultimos: n }); } else { await runAnuncios({ ...g, ultimos: n }); }
});

addGlobalFlags(
  canvas
    .command("inbox [id]")
    .description("Ver inbox Canvas (alias: wiener inbox)")
).action(async (id, opts) => {
  const g = parseGlobalFlags(opts);
  if (id) { await runInboxInfo(String(id), g); } else { await runInbox(g); }
});
