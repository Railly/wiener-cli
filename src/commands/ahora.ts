  ...
  program
    ...
    .action(async (opts: { json?: boolean }) => {
      ...
      try {
        ...
        const bloques = ((horario.dias as Record<string, HorarioBloque[] | undefined>)[diaKey] ??
          []) as HorarioBloque[];
        ...
        if (ahora) {
          ...
          lines.push(
            `         ${ahora.time_start}-${ahora.time_end} · ${ahora.type} · ${ahora.room}`,
          );
        } else {
        ...
        if (proximo) {
          ...
          lines.push(
            `${label}  ${proximo.course_code} · ${proximo.course_name}  ${color ? pc.yellow(`(en ${etaStr})`) : `(en ${etaStr})`}`,
          );
          lines.push(
            `         ${proximo.time_start}-${proximo.time_end} · ${proximo.type} · ${proximo.room}`,
          );
        }
        lines.push("");
