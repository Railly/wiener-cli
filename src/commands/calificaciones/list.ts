...
import { renderSection, renderTable } from "../../lib/output/human.js";
...
export async function runCalificaciones(opts: { json?: boolean; fields?: string }): Promise<void> {
  try {
    ...
    const rows = cursos.map((c) => ({
      codigo: c.code,
      nombre: c.name,
      nota_actual: gradeColor(
        c.current_grade ?? (c.current_score !== null ? String(c.current_score) : null),
      ),
      puntaje: c.current_score !== null ? String(c.current_score.toFixed(1)) : pc.dim("—"),
      nota_final: gradeColor(
        c.final_grade ?? (c.final_score !== null ? String(c.final_score) : null),
      ),
    }));
