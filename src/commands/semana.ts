      ...
      try {
        ...
        for (const ev of [...upcomingEvents, ...todo.filter((t) => t.assignment)]) {
          ...
          if (due >= now && due <= weekEnd) {
            ...
            const isQuiz =
              (ev.assignment?.submission_types ?? []).includes("online_quiz") ||
        ...
        if (tareas.length > 0) {
          ...
          for (const t of tareas) {
            const due = new Date(t.due_at).toLocaleDateString("es-PE", {
              weekday: "short",
              day: "2-digit",
              month: "short",
            });
        ...
        if (quizzes.length > 0) {
          ...
          for (const q of quizzes) {
            const due = new Date(q.due_at).toLocaleDateString("es-PE", {
              weekday: "short",
              day: "2-digit",
              month: "short",
            });
