...
import { getUpcomingEvents, getTodo } from "../api/canvas/calendar.js";
import { loadState, isStateStale, stateAgeLabel } from "../state/snapshot.js";
...
export async function buildPanorama(opts: {
  ...
  let pendienteHoy: PendienteItem[] = [];
  let estaSemana: EstaSemana = { tareas_count: 0, quizzes_count: 0 };
  ...
  try {
    ...
    for (const ev of allEvents) {
      ...
      const isQuiz = submissionTypes.some((t) => t === "online_quiz") ||
